...
#error "004"
#endif
#if (KRNLTMRVECTOR == TIMER0_OVF_vect)
#error "000 ovf"
#endif
...
#if (KRNLTMR == 0)
// krnltmr 0 == standard timer which also maintain  millis counter
...
struct k_t *
k_crt_task (void (*pTask) (void), char prio,  int stkSize)
{
  struct k_t *pT;
  char *pStk;
...
// the following code is ignored if we use timer0
#if (KRNLTMR !=0)
  TCCRxA = 0;
